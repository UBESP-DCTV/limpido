...
mixdb_otiti_tagged <- gold_otiti %>%
    ...
    mixdb(meta_vars(
        ...
        id_medico, descrizione_1, risultato_1, descrizione_2,
...
cache_ngram_path <- here("outputs/mixdb_otiti_tagged_ngram.rda")
if (file.exists(cache_ngram_path)) {
    load(cache_ngram_path)
} else {
    mixdb_otiti_tagged_ngram <- add_ngram(
        mixdb_otiti_tagged, ngram_range
    )
    save(mixdb_otiti_tagged_ngram, file = cache_ngram_path)
}

random_seed <- sample.int(1e4, 1)
set.seed(random_seed)

sets_len <- table(attr(mixdb_otiti_tagged_ngram, "meta")$set)
admitted_indeces  <- seq_along(sum(sets_len))
all_valid_indeces <- which(
    attr(mixdb_otiti_tagged_ngram, "meta")$set == "validation"
)
tuning_len <- 300L

tuning_indeces <- sample(x = all_valid_indeces, size = tuning_len)
train_indeces <- admitted_indeces[-tuning_indeces]
