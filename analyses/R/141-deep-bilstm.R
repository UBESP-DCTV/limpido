...
# Model definition ================================================
run_name <- "141-deep-bilstm"
architecture <- glue::glue("
    (filename: {run_name})

    embedding +
    conv2x256 + batch_norm +
    bi-lstm128w/sequence +
    dropout0.2 +
    bi-lstm64 +
    fc6_softmax
")
...
l3_rnn <- l2_conv2 %>%
    bidirectional(
      layer_lstm(units = 128L, return_sequences = TRUE),
      name = "l3_lstm"
    ) %>%
    layer_dropout(0.2) %>%
    bidirectional(layer_lstm(units = 64L), name = "l4_lstm")
