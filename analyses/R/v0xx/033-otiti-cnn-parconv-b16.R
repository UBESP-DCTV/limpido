...
architecture <- glue::glue("
    fixed pedianet embedding + conv(2+3+4)*256 +
    batchnorm + max_pool2 + dropout0.5 +
    fc256_relu + batchnorm + dropout0.5 +
    fc6_softmax
")
...
l1 <- layer_concatenate(
    ...
    layer_global_max_pooling_1d(name = "l1_global_maxpool")
...
save_all_models(
  model, history, params, preds, plot = p, start_time, fine_tuned = TRUE
)
